...
if __name__ == '__main__':
    ...
    start_epoch = 1
    # try:
    #     print(f'loading log: log/{args.try_no}.json')
    #     log = eval(open(f'log/{args.try_no}.json', 'r').read())
    ...
    criterion = nn.CrossEntropyLoss().cuda()  # todo: BCE might help bad image
    zz = 0
    for i in range(len(args.epochs)):
        ...
        def train(epoch):
            ...
            optimizer.zero_grad()
            criterion.zero_grad()
            progbar = generic_utils.Progbar(len(trainloader))
        ...
        def test(epoch):
            ...
            model.eval()
            test_loss = 0
            progbar = generic_utils.Progbar(len(val_loader))
